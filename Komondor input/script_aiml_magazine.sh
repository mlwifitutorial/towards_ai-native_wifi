...
# get input files path
  ...
  for (( executing_ix_nodes=0; executing_ix_nodes < (nodes_file_ix + 1); executing_ix_nodes++))
    do 
		...
		./komondor_main ../input/input_aiml_magazine_paper/obsspd_${OBSSPD}/${array_nodes[executing_ix_nodes]} ../output/script_output_aiml_magazine_obsspd_${OBSSPD}.txt sim_${executing_ix_nodes}_obsspd_${OBSSPD} 0 1 1 $SIM_TIME $SEED >> ../output/logs_console.txt
  done
  cd ..
done
