...
# get input files path
  ...
  for (( executing_ix_nodes=0; executing_ix_nodes < (nodes_file_ix + 1); executing_ix_nodes++))
    do 
		...
		./komondor_main ../input/input_aiml_magazine_paper/obsspd_${OBSSPD}/${array_nodes[executing_ix_nodes]} ../input/input_aiml_magazine_paper/agents_obsspd.csv ../output/script_output_aiml_magazine_agents_obsspd.txt sim_${executing_ix_nodes}_agents_obsspd 0 0 0 1 1 $SIM_TIME $SEED >> ../output/logs_console.txt
  done
  cd ..
done
